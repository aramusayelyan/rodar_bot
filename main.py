...
def start(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(
        (
            ...
            "Կարող եք սեղմել կոճակը՝ կոնտակտ ուղարկելու համար։"
        ),
...
def got_phone(update: Update, context: CallbackContext):
    ...
    if not phone:
        update.message.reply_text("Հեռախոսահամարը սխալ է։ Ուղարկեք +374XXXXXXXX կամ 0XXXXXXXX ձևաչափով։")
        return ASK_PHONE
    st = _state(context)
    st["phone"] = phone
    update.message.reply_text(
        "Մուտքագրեք Ձեր սոցիալական քարտի համարը (ՀԾՀ, 10 թվանշան)։",
...
def got_social(update: Update, context: CallbackContext):
    ...
    if not (social.isdigit() and len(social) == 10):
        update.message.reply_text("Խնդրում ենք ուղարկել հենց 10 թվանշան (օր. 1234567890)։")
        return ASK_SOCIAL
    st = _state(context)
    st["social"] = social
...
def request_login_and_ask_code(update: Update, context: CallbackContext, sess: requests.Session):
    ...
    try:
        ...
        update.message.reply_text("✅ SMS հաստատման կոդը ուղարկվեց։ Խնդրում ենք ուղարկել ստացած թվային կոդը։")
        return WAIT_SMS_CODE
    except Exception as e:
        logger.warning("login_init failed: %s", e)
        update.message.reply_text("Չհաջողվեց ուղարկել կոդը։ Փորձեք կրկին /search։")
        return ConversationHandler.END

def got_sms_code(update: Update, context: CallbackContext):
    code = (update.message.text or "").strip()
    if not (code.isdigit() and (4 <= len(code) <= 8)):
        update.message.reply_text("Ուղարկեք ճիշտ թվային կոդը։")
        return WAIT_SMS_CODE
    user_id = update.effective_user.id
    u = db.get_user(user_id)
    if not u:
    ...
    try:
        ...
        if not services:
            update.message.reply_text("Չհաջողվեց բեռնել ծառայությունները։ Փորձեք /search։")
            return ConversationHandler.END
        st = _state(context)
        st["branches"] = branches
        st["services_all"] = services
        st["chosen"] = {}
        update.message.reply_text("Մուտքը հաստատվեց ✅\nԸնտրեք քննության տեսակը․", reply_markup=keyboards.exam_type_keyboard())
        return SEARCH_EXAM
    except Exception as e:
        logger.warning("login_verify failed: %s", e)
        update.message.reply_text("Կոդը սխալ է կամ ժամկետն անցել է։ Կատարեք կրկին /search։")
        return ConversationHandler.END
...
def do_nearest(update: Update, context: CallbackContext):
    ...
    if slots:
        ...
        update.message.reply_text(f"Ամենամոտ օրը՝ {day}\nԸնտրեք ժամերից մեկը՝", reply_markup=keyboards.slot_inline_keyboard(slots))
...
def do_all_days(update: Update, context: CallbackContext):
    ...
    _save_cookies(user_id, sess)
    txt = "Անընդունելի օրեր՝ " + (", ".join(disabled) if disabled else "—")
...
def slot_clicked(update: Update, context: CallbackContext):
    ...
    if data.startswith("follow|on"):
        user_id = update.effective_user.id
        db.upsert_tracker(user_id, st["chosen"]["service_id"], st["chosen"]["branch_id"], context.user_data.get("last_nearest"))
...
def ask_email_done(update: Update, context: CallbackContext):
    ...
    sess, _ = _get_session(user_id)
    try:
        res = scraper.register(sess, st["chosen"]["branch_id"], st["chosen"]["service_id"], st["chosen"]["date"], st["chosen"]["slot_time"], email)
        _save_cookies(user_id, sess)
        pin = res.get("pin") or "—"
        update.message.reply_text(f"✅ Գրանցումն ավարտվեց\nPIN: {pin}")
        update.message.reply_text("Սկսե՞լ հետևել ամենամոտ օրվան այս բաժնի/ծառայության համար:", reply_markup=keyboards.confirm_follow_keyboard())
        return WAIT_SLOT_SELECT
    except Exception as e:
    ...
    try:
        ...
        for t in trackers:
            ...
            if day and (last is None or day < last):
                try:
                    bot.send_message(chat_id=user_id, text=f"🔔 Գտնվեց ավելի մոտ օր՝ {day}\n/search հրամանով կարող եք ամրագրել։")
                except Exception:
                    pass
...
def main():
    ...
    if not (config.WEBHOOK_BASE_URL and config.PORT):
        raise RuntimeError("WEBHOOK_BASE_URL and PORT environment variables are required.")
    ...
    updater.start_webhook(listen="0.0.0.0", port=config.PORT, url_path=webhook_path, webhook_url=webhook_url)
